...
| Task | Tool | Why |
|------|------|-----|
| Competitor homepage/PDP content | **Firecrawl** (JSON mode) | Fast structured extraction, handles Shopify well |
| Competitor Amazon reviews | **Apify** (`apify/amazon-reviews-scraper`) | Clean structured review data with ratings, dates, verified status |
| Competitor social media presence | **Apify** (`apify/instagram-scraper`, `apify/tiktok-scraper`) | Follower counts, post engagement, content themes |
| Reddit/forum discussions | **Apify** (`apify/reddit-scraper`) | Extract threads about the product category |
| Mobile + desktop screenshots | **Playwright** | Full scroll-through screenshots at exact viewports |
| Meta Ad Library | **Playwright** + `/meta-ads` skill | Scrape active ad creatives and copy |
| Page speed/performance | **Firecrawl** or direct Lighthouse | Core web vitals |
| Review widgets on Shopify | **Firecrawl** (with `waitFor: 8000`) | Loads JS-rendered review widgets |
| TikTok/Instagram ad scraping | **Apify** (`apify/tiktok-scraper`) | Structured post data with engagement |
